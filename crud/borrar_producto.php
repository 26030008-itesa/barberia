...
// 1. Validar usuario en BD usando SP
try {
    ...
    // Limpiar resultados para la siguiente llamada (CRÍTICO)
    while($conn->more_results() && $conn->next_result()){;}

    // 2. Validar consistencia de Rol
    ...
    if ($rol_actual_sesion != $rol_en_db_app) {
        session_unset(); session_destroy();
...
$cveProducto = $_GET['clv'] ?? null;
$mensaje = '';

if ($cveProducto) {
    try {
        // Llamada al SP de Borrado Seguro
        // Nota: La conexión ya fue limpiada arriba
        $stmt = $conn->prepare("CALL sp_borrar_producto_seguro(?, ?)");
        $stmt->bind_param("is", $cveProducto, $currentUser);
        
        if ($stmt->execute()) {
            // Verificar si hubo error de negocio (DENEGADO)
            // Para eso necesitamos capturar advertencias o usar una variable de salida,
            // pero el SIGNAL SQLSTATE del SP lanzará una excepción que atrapamos abajo.
            $mensaje = "Producto CLV #$cveProducto eliminado correctamente.";
        }
        $stmt->close();

    } catch (mysqli_sql_exception $e) {
        // Capturamos el mensaje del SIGNAL SQLSTATE '45000' del SP
...
<body class="fondo-crud">
    <div class="login-container">
        ...
        <a href="productos.php" class="btn-principal">Volver a la lista</a>
