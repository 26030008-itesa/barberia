...
while($conn->more_results() && $conn->next_result()){;}
$stmt = $conn->prepare("CALL sp_listar_empleados_filtro(?, ?)");
...
<head>
    ...
    <title>Visago - Gestión de Barberos</title>
...
<body class="fondo-empleados">
    <div class="app-container">
        ...
        <aside class="sidebar collapsed" id="sidebar">
            ...
            <?php if ($rol == 'admin'): ?>
                <a href="empleados.php" class="nav-link active"><svg class="svg-icon" viewBox="0 0 24 24"><rect x="8" y="2" width="8" height="20" rx="1"></rect><path d="M8 6h8M8 10h8M8 14h8M8 18h8"></path></svg><span class="nav-link-text">Barberos</span></a>
        ...
        <main class="main-content">
            ...
            <header class="topbar">
                <div class="topbar-greeting">Visago Studio <span style="color:var(--c-text-muted); font-weight:normal; font-family:'Montserrat', sans-serif;">/ Gestión de Barberos</span></div>
            ...
            <div class="crud-container" style="margin-top: 0; flex-grow: 1;">
                <h2>Gestión de Barberos / Empleados</h2>
                
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    
                    <div class="search-container" style="margin: 0; padding: 0; background: transparent; border: none; flex-grow: 1;">
                        <form method="GET" action="empleados.php" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            ...
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <label style="color: var(--c-text-muted); font-weight: 600; font-size: 0.9em; text-transform: uppercase;">Ingreso:</label>
                            ...
                            <a href="empleados.php" style="color: var(--c-text-muted); text-decoration: none; font-weight: bold; font-size: 0.9em; transition: 0.3s;">Limpiar Filtros</a>
                        </form>
                    </div>

                    <a href="crear_empleado.php" class="btn-new">+ Agregar Barbero</a>
                </div>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--c-text-cream); font-weight: bold;"><?= htmlspecialchars($row['nombre1'] . ' ' . $row['apellidoP']) ?></td>
                                    <td style="color: var(--c-text-muted);"><?= htmlspecialchars($row['fechaIngreso']) ?></td>
                                    <td>
                                        <a href="editar_empleado.php?clv=<?= htmlspecialchars($row['clv']) ?>">Editar</a> 
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        <a href="borrar_empleado.php?clv=<?= htmlspecialchars($row['clv']) ?>" 
                                           onclick="return confirm('¿Estás seguro de eliminar al barbero <?= htmlspecialchars($row['nombre1']) ?>?')" style="color: #D9534F;">Borrar</a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="4" style="text-align: center; padding: 40px; color: var(--c-text-muted);">No se encontraron empleados registrados.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
