...
// Validación de Rol (Admin o Vendedor)
$rol = $_SESSION['rol'];
...
while($conn->more_results() && $conn->next_result()){;}
$stmt = $conn->prepare("CALL sp_listar_productos_filtro(?)");
...
<head>
    ...
    <title>Visago - Inventario de Productos</title>
    ...
    <div class="app-container">
        ...
        <aside class="sidebar collapsed" id="sidebar">
            ...
            <?php if (in_array($rol, ['admin', 'vendedor'])): ?>
                ...
                <a href="productos.php" class="nav-link active"><svg class="svg-icon" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg><span class="nav-link-text">Productos (Inventario)</span></a>
            <?php endif; ?>
        ...
        <main class="main-content">
            ...
            <header class="topbar">
                <div class="topbar-greeting">Visago Studio <span style="color:var(--c-text-muted); font-weight:normal; font-family:'Montserrat', sans-serif;">/ Inventario de Productos</span></div>
            ...
            <div class="crud-container" style="margin-top: 0; flex-grow: 1;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    ...
                    <div class="search-container" style="margin: 0; padding: 0; background: transparent; border: none; flex-grow: 1;">
                        <form method="GET" action="productos.php" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            ...
                            <a href="productos.php" style="color: var(--c-text-muted); text-decoration: none; font-weight: bold; font-size: 0.9em; transition: 0.3s;">Limpiar Filtros</a>
                        </form>
                    </div>

                    <?php if ($rol == 'admin'): ?>
                        <a href="crear_producto.php" class="btn-new">+ Agregar Nuevo Producto</a>
                    <?php endif; ?>
                </div>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--c-text-cream); font-weight: 600;"><?= htmlspecialchars($row['nombre']) ?></td>
                                    <td style="color: var(--c-text-muted); font-size: 0.85em;"><?= htmlspecialchars(substr($row['descripcion'], 0, 50)) . (strlen($row['descripcion']) > 50 ? '...' : '') ?></td>
                                    <td style="color: var(--c-accent-gold); font-weight: bold; font-size: 1.1em;">$<?= htmlspecialchars(number_format($row['precio'], 2)) ?></td>
                                    
                                    <td style="color: var(--c-text-cream); font-weight: bold;">
                                        <?= htmlspecialchars($row['stock']) ?>
                                        <?php if((int)$row['stock'] <= 5): ?>
                                            <span style="color: #D9534F; font-size: 0.8em; margin-left: 5px;">(Bajo)</span>
                                        <?php endif; ?>
                                    </td>
                                    
                                    <td style="color: var(--c-text-muted);"><?= htmlspecialchars($row['nombre_proveedor']) ?></td>
                                    
                                    <?php if ($rol == 'admin'): ?>
                                    <td>
                                        <a href="editar_producto.php?clv=<?= htmlspecialchars($row['clv']) ?>">Editar</a> 
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        
                                        <a href="borrar_producto.php?clv=<?= htmlspecialchars($row['clv']) ?>" 
                                           onclick="return confirm('¿Estás seguro de que quieres eliminar el producto <?= htmlspecialchars($row['nombre']) ?>?')" style="color:#D9534F;">Borrar</a>
                                    </td>
                                    <?php endif; ?>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="<?= ($rol == 'admin') ? '7' : '6' ?>" style="text-align: center; padding: 40px; color: var(--c-text-muted);">No se encontraron productos en el inventario.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
