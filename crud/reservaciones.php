...
if (!in_array($rol, ['admin', 'vendedor', 'empleado'])) { header("Location: ../dashboard.php"); exit; }

$search_clv = $_GET['search_clv'] ?? '';
$search_fecha = !empty($_GET['search_fecha']) ? $_GET['search_fecha'] : null;

// Lógica de cancelación
if (isset($_GET['accion']) && $_GET['accion'] == 'cancelar_sp' && isset($_GET['id'])) {
    $id_cancelar = $_GET['id'];
    try {
        $stmt_sp = $conn->prepare("CALL sp_cancelar_cita(?)");
        $stmt_sp->bind_param("i", $id_cancelar);
        if ($stmt_sp->execute()) {
            echo "<script>alert('✅ Cita cancelada correctamente (SP).'); window.location.href='reservaciones.php';</script>"; exit;
        }
...
// Búsqueda principal
while($conn->more_results() && $conn->next_result()){;}
$stmt = $conn->prepare("CALL sp_listar_reservaciones_filtro(?, ?)");
$stmt->bind_param("ss", $search_clv, $search_fecha);
$stmt->execute();
...
<head>
    ...
    <title>Visago - Agenda de Citas</title>
    <link rel="stylesheet" href="../css/style.css"> 
</head>
<body class="fondo-reservaciones">
    <div class="app-container">
        ...
        <aside class="sidebar collapsed" id="sidebar">
            ...
            <a href="reservaciones.php" class="nav-link active">
        ...
        <main class="main-content">
            ...
            <header class="topbar">
                <div class="topbar-greeting">Visago Studio <span style="color:var(--c-text-muted); font-weight:normal; font-family:'Montserrat', sans-serif;">/ Agenda de Citas</span></div>
            ...
            <div class="crud-container" style="margin-top: 0; flex-grow: 1;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    ...
                    <div class="search-container" style="margin: 0; padding: 0; background: transparent; border: none; flex-grow: 1;">
                        <form method="GET" action="reservaciones.php" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            <input type="text" name="search_clv" placeholder="Buscar por CLV..." value="<?= htmlspecialchars($search_clv) ?>" style="padding: 12px; border: 1px solid #333; border-radius: 8px; background: rgba(0,0,0,0.3); color: var(--c-text-cream); outline: none; width: 200px;">
                            
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <label style="color: var(--c-text-muted); font-weight: 600; font-size: 0.9em; text-transform: uppercase;">Fecha:</label>
                                <input type="date" name="search_fecha" value="<?= htmlspecialchars($search_fecha ?? '') ?>" style="padding: 10px; border: 1px solid #333; border-radius: 8px; background: rgba(0,0,0,0.3); color: var(--c-text-cream); outline: none; color-scheme: dark;">
                            </div>
                            ...
                            <a href="reservaciones.php" style="color: var(--c-text-muted); text-decoration: none; font-weight: bold; font-size: 0.9em; transition: 0.3s;">Limpiar Filtros</a>
                        </form>
                    </div>

                    <?php if (in_array($rol, ['admin', 'vendedor'])): ?>
                        <a href="crear_reservacion.php" class="btn-new">+ Agendar Nueva Cita</a>
                    <?php endif; ?>
                </div>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--c-text-cream);"><?= htmlspecialchars($row['cliente_nombre'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['servicio_nombre'] ?? '') ?></td>
                                    <td style="color: var(--c-text-muted);"><?= htmlspecialchars($row['empleado_nombre'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['fecha_reserva']) ?> <span style="color: var(--c-accent-gold);">@</span> <?= htmlspecialchars(substr($row['hora_reserva'], 0, 5)) ?></td>
                                    <td>
                                        <?php
                                            // Aplicamos los estilos de estado que ya están en el CSS
                                            $est = htmlspecialchars($row['estado']);
                                            echo "<span class='estado-".strtolower(str_replace(' ','',$row['estado']))."'>$est</span>";
                                        ?>
                                    </td>
                                    <td>
                                        <a href="editar_reservacion.php?clv=<?= htmlspecialchars($row['clv']) ?>">Editar</a>
                                        
                                        <?php if ($row['estado'] != 'cancelada'): ?>
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        <a href="reservaciones.php?accion=cancelar_sp&id=<?= htmlspecialchars($row['clv']) ?>" 
                                        onclick="return confirm('¿Cancelar cita #<?= htmlspecialchars($row['clv']) ?>?')"
                                        style="color: #E6A23C;">Cancelar</a>
                                        <?php endif; ?>

                                        <?php if ($rol == 'admin'): ?>
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        <a href="borrar_reservacion.php?clv=<?= htmlspecialchars($row['clv']) ?>" 
                                        onclick="return confirm('¿Eliminar permanentemente?')" style="color: #D9534F;">Borrar</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--c-text-muted);">No se encontraron reservaciones para estos filtros.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
