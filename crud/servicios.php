...
if (!in_array($rol, ['admin', 'empleado'])) { header("Location: ../dashboard.php"); exit; }
...
while($conn->more_results() && $conn->next_result()){;}
$stmt = $conn->prepare("CALL sp_listar_servicios_filtro(?)");
...
<head>
    ...
    <title>Visago - Catálogo de Servicios</title>
    ...
    <div class="app-container">
        ...
        <aside class="sidebar collapsed" id="sidebar">
            ...
            <?php if ($rol == 'admin' || $rol == 'empleado'): ?>
                <a href="servicios.php" class="nav-link active"><svg class="svg-icon" viewBox="0 0 24 24"><circle cx="6" cy="6" r="3"></circle><circle cx="6" cy="18" r="3"></circle><line x1="20" y1="4" x2="8.12" y2="15.88"></line><line x1="14.47" y1="14.48" x2="20" y2="20"></line><line x1="8.12" y1="8.12" x2="12" y2="12"></line></svg><span class="nav-link-text">Servicios</span></a>
            <?php endif; ?>
        ...
        <main class="main-content">
            ...
            <header class="topbar">
                <div class="topbar-greeting">Visago Studio <span style="color:var(--c-text-muted); font-weight:normal; font-family:'Montserrat', sans-serif;">/ Catálogo de Servicios</span></div>
            ...
            <div class="crud-container" style="margin-top: 0; flex-grow: 1;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    ...
                    <div class="search-container" style="margin: 0; padding: 0; background: transparent; border: none; flex-grow: 1;">
                        <form method="GET" action="servicios.php" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            ...
                            <a href="servicios.php" style="color: var(--c-text-muted); text-decoration: none; font-weight: bold; font-size: 0.9em; transition: 0.3s;">Limpiar Filtros</a>
                        </form>
                    </div>

                    <?php if ($isAdmin): ?>
                        <a href="crear_servicio.php" class="btn-new">+ Nuevo Servicio</a>
                    <?php endif; ?>
                </div>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--c-text-muted);"><?= htmlspecialchars(substr($row['descripcion'], 0, 80)) . (strlen($row['descripcion']) > 80 ? '...' : '') ?></td>
                                    <td style="color: var(--c-accent-gold); font-weight: bold; font-size: 1.1em;">$<?= htmlspecialchars(number_format($row['precio'], 2)) ?></td>
                                    
                                    <?php if ($isAdmin): ?>
                                    <td>
                                        <a href="editar_servicio.php?clv=<?= htmlspecialchars($row['clv']) ?>">Editar</a> 
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        <a href="borrar_servicio.php?clv=<?= htmlspecialchars($row['clv']) ?>" 
                                           onclick="return confirm('¿Estás seguro de eliminar el servicio <?= htmlspecialchars($row['nombre']) ?>?')" style="color: #D9534F;">Borrar</a>
                                    </td>
                                    <?php endif; ?>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="<?= ($isAdmin) ? '5' : '4' ?>" style="text-align: center; padding: 40px; color: var(--c-text-muted);">No se encontraron servicios registrados.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
