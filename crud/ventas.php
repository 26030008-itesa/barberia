...
if (!in_array($rol, ['admin', 'vendedor'])) { header("Location: ../dashboard.php"); exit; }
...
while($conn->more_results() && $conn->next_result()){;}
$stmt = $conn->prepare("CALL sp_listar_ventas_filtro(?, ?)");
$stmt->bind_param("ss", $search_clv, $search_fecha);
$stmt->execute();
$result = $stmt->get_result();

$mensaje = $_SESSION['mensaje_status'] ?? '';
unset($_SESSION['mensaje_status']);
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Visago - Historial de Ventas</title>
    <link rel="stylesheet" href="../css/style.css"> 
</head>
<body class="fondo-ventas">
    <div class="app-container">
        ...
        <aside class="sidebar collapsed" id="sidebar">
            ...
            <?php if (in_array($rol, ['admin', 'vendedor'])): ?>
                ...
                <a href="ventas.php" class="nav-link active"><svg class="svg-icon" viewBox="0 0 24 24"><rect x="2" y="6" width="20" height="12" rx="2"></rect><circle cx="12" cy="12" r="2"></circle><path d="M6 12h.01M18 12h.01"></path></svg><span class="nav-link-text">Ventas</span></a>
        ...
        <main class="main-content">
            ...
            <header class="topbar">
                <div class="topbar-greeting">Visago Studio <span style="color:var(--c-text-muted); font-weight:normal; font-family:'Montserrat', sans-serif;">/ Ventas</span></div>
            ...
            <div class="crud-container" style="margin-top: 0; flex-grow: 1;">
                ...
                <?php if (!empty($mensaje)): ?>
                    <div class="msg-success" style="margin-bottom: 20px;">✓ <?= htmlspecialchars($mensaje) ?></div>
                <?php endif; ?>
                
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    
                    <div class="search-container" style="margin: 0; padding: 0; background: transparent; border: none; flex-grow: 1;">
                        <form method="GET" action="ventas.php" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            <input type="text" name="search_clv" placeholder="Buscar por ticket..." value="<?= htmlspecialchars($search_clv) ?>" style="padding: 12px; border: 1px solid #333; border-radius: 8px; background: rgba(0,0,0,0.3); color: var(--c-text-cream); outline: none; width: 200px;">
                            
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <label style="color: var(--c-text-muted); font-weight: 600; font-size: 0.9em; text-transform: uppercase;">Fecha:</label>
                                <input type="date" name="search_fecha" id="search_fecha" value="<?= htmlspecialchars($search_fecha ?? '') ?>" style="padding: 10px; border: 1px solid #333; border-radius: 8px; background: rgba(0,0,0,0.3); color: var(--c-text-cream); outline: none; color-scheme: dark;">
                            </div>
                            ...
                            <a href="ventas.php" style="color: var(--c-text-muted); text-decoration: none; font-weight: bold; font-size: 0.9em; transition: 0.3s;">Limpiar Filtros</a>
                        </form>
                    </div>

                    <a href="crear_venta.php" class="btn-new">+ Registrar Venta</a>
                </div>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--c-accent-gold); font-weight: bold; font-size: 1.1em;">$<?= htmlspecialchars(number_format($row['total'], 2)) ?></td>
                                    <td>
                                        <?php
                                            $est = htmlspecialchars($row['estado']);
                                            echo "<span class='estado-".strtolower($row['estado'])."'>$est</span>";
                                        ?>
                                    </td>
                                    <td>
                                        <a href="ver_venta.php?clv=<?= htmlspecialchars($row['clv']) ?>">Ver Detalle</a> 
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        
                                        <a href="editar_venta.php?clv=<?= htmlspecialchars($row['clv']) ?>">Editar</a> 
                                        <span style="color:#333; margin:0 5px;">|</span>
                                        
                                        <a href="borrar_venta.php?clv=<?= htmlspecialchars($row['clv']) ?>" 
                                           onclick="return confirm('¿Seguro que deseas anular la venta #<?= htmlspecialchars($row['clv']) ?>?')" style="color:#D9534F;">Borrar</a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="6" style="text-align: center; padding: 40px; color: var(--c-text-muted);">No se encontraron ventas para estos filtros.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
